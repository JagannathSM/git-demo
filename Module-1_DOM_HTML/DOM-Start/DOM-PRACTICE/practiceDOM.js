...
var auto = fetch("https://raw.githubusercontent.com/Rajavasanthan/jsondata/master/pagenation.json")
var objarr = auto.then((data)=>{
    return data.json()
}).then((data1)=> data1);
var length = objarr.then((data2) => console.log(data2.length));
...
var page_button_start = document.createElement("div");
page_button_start.className = "hello";
...
var total_page = 5;
var page_per_ele = 10;
var total_data_api = 100; //need to enter total API data
var total_page_need = Math.ceil(total_data_api / page_per_ele);
function create_buttons_dynamic(start_page,end_page){
    if(end_page<=total_page_need){
        for(i=start_page;i<=total_page;i++){
            var i_val = String(i);
            var btn_page_dynamic = buttons1 ("class","page",i_val,"click",indexpage_fun,"id",i_val);
            page_button_div.append(btn_page_dynamic);
        }
    }
    page_button_start.append(page_button_div);
}


tabel_footer_last.append(prev_btn,page_button_start,next_btn);
...
function setactive(start_page,end_page){
    create_buttons_dynamic(start_page,end_page);
...
document.onload(setactive(start_page,end_page));
