...
function UpdateStudent() {
    ...
    const getValues = async(_id) =>{
        if(!studentName || !currentMentorID){
            ...
            try{
                await axios.put(`https://assign-mentor-backend-pws4.onrender.com/student/update/${_id}`,{studentName,currentMentor:currentMentorID});
                navigat(-1);
            } catch (err) {
                alert(`Warning message : ${err.response.data.message}. Only studentName will update`)
            }
        }
    }
    ...
    const getStudentData = async(_id) =>{
        try{
            const { data } = await axios.get(`https://assign-mentor-backend-pws4.onrender.com/student/show/${_id}`);
    ...
    if(!mentors || !studentName) return <div>Loading Mentor Assign Page for Selected Student</div>
  ...
  return (
    ...
    <div>
        ...
        <Box component="form" sx={{ m: 1, width: '25ch' }} noValidate>
            <TextField id="StudentName" label="Student Name" value={studentName} variant="outlined" required onChange={(e)=>setStudentName(e.target.value)}/>
        ...
        <Button variant="contained" onClick={()=>{getValues(_id)}}>Get Values</Button>
        </Box>
    </div>
    </>
  )
